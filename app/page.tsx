...
export default function Home() {
  ...
  const serviceQuery = searchParams.get("service"); // e.g. "UI/UX"
  ...
  const {
    register,
    handleSubmit,
    setValue, // Needed for programmatically setting values
  ...
  // Prefill services from ?service= query param
  useEffect(() => {
    if (serviceQuery) {
      setValue("services", [decodeURIComponent(serviceQuery)]);
    }
      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
        <div className="bg-green-100 border border-green-300 p-6 rounded-lg shadow-md max-w-md w-full text-center">
          ...
          <p className="text-gray-700">
            Thanks {submitSuccess.fullName}, we received your request.
          </p>
        </div>
      </div>
    );
